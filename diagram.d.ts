...
declare type SelectionChangedFunc = (shapeId: string) => void;

interface IDiagram {
    shapes: { [key: string]: IShape };

    setSelection(shapeId: string);
    getSelectedShapeId(): string;
    selectionChanged: JQueryCallback;

    pages: IPage[];
    currentPage: IPage;

    layers: ILayer[];
    setLayerVisible(layerName: string, set: boolean);
    isLayerVisible(layerName: string): boolean;
}

declare const diagram: IDiagram;
